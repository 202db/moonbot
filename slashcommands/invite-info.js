...
module.exports = {
    ...
    options: [
        {
          type: "string",
          name: "invite",
          description: "Lien ou code d'invitation.",
          required: true,
          autocomplete: false,
        },
      ],
    async run(bot, message, args) {
        try {
            const invite = await bot.fetchInvite(args.getString("invite"))
            const guild = invite.guild
            const embed = new Discord.EmbedBuilder()
            .setAuthor({ name: `Informations sur l'invitation`, url: "https://discord.gg/WzQGA8A5", iconURL: bot.user.displayAvatarURL({ dynamic: true })})
            .setTitle(guild.name)
            .addFields(
