    ...
    async run(bot, message, args) {
        bot.db.query(`SELECT * FROM bot WHERE guildId = "${message.guild.id}"`, async (err, req) => {
            ...
            const confirmationEmbed = new EmbedBuilder()
                .setColor("White")
                .setAuthor({ name: 'Expulsion des membres récents', iconURL: bot.user.displayAvatarURL(), url: "https://discord.gg/WzQGA8A5" })
            ...
            collector.on('collect', async i => {
                ...
                if (i.customId === 'confirm_button') {
                    membersToPrune.forEach(async member => {
                        try {
                            await member.kick(`Expulsion des membres récents par @${message.user.username}`);
                        } catch (error) {
                        }
                    });
