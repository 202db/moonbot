...
module.exports = {
  ...
  async execute(oldMember, newMember, bot) {
    bot.db.query(`SELECT * FROM utilitaire WHERE guildId = "${newMember.guild.id}"`, async (err, req) => {
        ...
        .setColor("#f47fff")
        .setAuthor({ name: `${newMember.guild.members.cache.get(newMember.id).user.username} vient de booster le serveur !`, iconURL: newMember.guild.members.cache.get(newMember.id).displayAvatarURL({ dynamic: true }), url: "https://discord.gg/WzQGA8A5" })
