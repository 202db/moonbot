            ...
            .then(async (collected) => {
                if(collected.first().mentions.users.first() || message.guild.members.cache.get(collected.first().content)) {
                    ...
                    bot.db.query(`SELECT * FROM logs WHERE guildId = "${message.guild.id}"`, async (err, req) => {
                        ...
                        .setAuthor({ name: `@${message.guild.members.cache.get(user.id).user.username}`, iconURL: message.guild.members.cache.get(user.id).displayAvatarURL({ dynamic: true }), url: "https://discord.gg/WzQGA8A5"})
                        .setTimestamp()
